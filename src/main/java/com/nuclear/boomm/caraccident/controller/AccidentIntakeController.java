...
public class AccidentIntakeController {
    ...
    @PostMapping()
    public ResponseEntity<ApiResponse<AccidentIntakeIdDTO>> acceptIntake(@RequestBody AccidentIntakeDTO dto) {
        Long userId = 1L;

        return ResponseEntity.status(HttpStatus.CREATED).body(intakeService.acceptIntake(dto, userId));
